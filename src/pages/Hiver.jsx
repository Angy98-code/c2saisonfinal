...
const FruitsStyled = styled.div`
  display: flex;
  background-color: rgba(234, 223, 208, 0.7);
  border-radius: 10px;
...
export default function Ete() {
  const title = "En hiver c'est 2 saison :";
  const fruitsHiver = dataSeason2.filter(
    (element) => element.type === "fruit" && element.season === "hiver"
  );

  const legumesHiver = dataSeason2.filter(
    (element) => element.type === "légume" && element.season === "hiver"
  );
  console.log("fruitsHiver", fruitsHiver);
  console.log("legumesHiver", legumesHiver);
    ...
    <div className="hivercontainer">
      ...
      <VegetableStyled>
        <LegumesStyled>
          ...
          {/* dataSeason2.filter  */}
          {legumesHiver.map((element, index) => (
            <AStyled
              key={`${element}-${index}`}
